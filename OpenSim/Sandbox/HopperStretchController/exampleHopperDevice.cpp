...
void addSignalGeneratorToDevice(Device& device)
{
	...
	signalGen->set_function(StepFunction(0., 2.5, .8, .1));
    ...
    device.updInput("controller/length_setpoint")
...
int main()
{
	try {
		...
		if (true)
		{
			...
			// be filled in.
			//kneeDevice->updInput("controller/length")
			//	.connect(kneeDevice->getOutput("length"));

			addSignalGeneratorToDevice(*kneeDevice);


			kneeDevice->updInput("controller/length")
				.connect(assistedHopper.getOutput("/device/length"));

			// List the device outputs we wish to display during the simulation.
			std::vector<std::string> kneeDeviceOutputs{ "length",
														"controller/stretch_control",
														"tension", "height" };
