...
Model buildTugofwarModel()
{
	...
	try {
		...
		block->attachGeometry(new Sphere(SPHERE_RADIUS));
		...
		muscle1->setOptimalForce(maxIsometricForce);
		PathActuator *muscle2 = new PathActuator();
		muscle2->setName("muscle2");
		muscle2->setOptimalForce(maxIsometricForce);
		...
		PathSpring* ps = new PathSpring("damper", 0., STIFFNESS, DAMPING);
		ps->updGeometryPath().appendNewPathPoint("muscle2-point1", ground, Vec3(0.0, 0.05, 0.35));
		ps->updGeometryPath().appendNewPathPoint("muscle2-point2", *block, Vec3(0.0, 0.0, 0.05));
		cout << ps->getDissipation() << endl;
		// Add the two muscles (as forces) to the model
		osimModel.addForce(muscle1);
		osimModel.addForce(muscle2);
		osimModel.addForce(ps);
